 ...
 * @LastEditTime: 2019-08-28 17:53:59
...
/**
	...
	load: function(el) {
		...
		$(el).find('[data-signle-async]').find("input,select,textarea,[contenteditable='true']").unbind('focus').on('focus',
				function(event) {
					app.tranceValue = $(this).attr('data-val') || $(this).val() || $(this).text() || $(this).html(); //传输的数据
				}),
			$(el).find('[data-signle-async]').find("input,select,textarea,[contenteditable='true']").unbind('focusout').on(
				'focusout',
				function(event) {
...
var addressPice = {
	...
	load: function() {
		...
		//触发change事件
		$('.address-group .address').click(function() {
			$(this).change()
		});
		//读取下级数据
		addressPice.setNextDate();
	...
	setCheckbox: function(selecter) {
		...
		$(selecter).each(function(index, item) {
			...
			for (var i = 0; i < key.length; i++) {
				...
				if (defVal.indexOf(key[i]) + 1) curCode.find("input[type='checkbox']").attr('checked', "checked").parent().addClass(
					'active');
